...
from .models import VocalCapturada, HistorialReconocimientoVocal, PracticaVocal, TipoVocal, TipoMano

logger = logging.getLogger(__name__)

# Vista auxiliar: redirige a la SPA del frontend en desarrollo
def frontend_capture_redirect(request):
    """
    Redirige /vocales/capture (en el backend) a la ruta del frontend
    cuando se accede directamente por el puerto 8000 durante desarrollo.
    """
    # Por defecto usamos el host local de Vite
    return redirect('http://localhost:5173/vocales/capture')

@csrf_exempt
@require_http_methods(["POST"])
def guardar_gesto(request):
    """API para guardar un nuevo gesto de vocal entrenado"""
    try:
        ...
        vocal_vinculada = data.get('vocal_vinculada')
        ...
        if not vocal_vinculada or not landmarks_data:
            return JsonResponse({'success': False,'error': 'Faltan datos requeridos: debe especificar vocal y landmarks'}, status=400)
        
        # Validar que la vocal sea válida
        vocales_validas = [choice[0] for choice in TipoVocal.choices]
        if vocal_vinculada not in vocales_validas:
            return JsonResponse({'success': False,'error': f'Vocal no válida: {vocal_vinculada}. Válidas: {vocales_validas}'}, status=400)

        nombre_display = f"Vocal {vocal_vinculada}"

        filter_params = {
            'vocal_vinculada': vocal_vinculada,
            'tipo_mano': tipo_mano
        }
        ...
        vocal, created = VocalCapturada.objects.update_or_create(
            **filter_params,
            defaults={
        ...
        return JsonResponse({
            'success': True,
            'message': f'Vocal "{nombre_display}" {"creada" if created else "actualizada"} exitosamente',
            'vocal_id': vocal.id,
            'created': created
        })
...
def reconocer_gesto(request):
    """API para reconocer un gesto de vocal capturado"""
    try:
        # Validar que el request tenga contenido
        if not request.body:
            return JsonResponse({'success': False, 'error': 'No se enviaron datos en la petición'}, status=400)
        ...
        vocales_entrenadas = VocalCapturada.objects.filter(activo=True)
        if not vocales_entrenadas.exists():
            return JsonResponse({'success': False,'error': 'No hay vocales entrenadas disponibles'}, status=400)

        mejor_coincidencia = None
        mejor_confianza = 0.0
        for vocal in vocales_entrenadas:
            confianza = calcular_similitud_landmarks(points, vocal.get_landmarks_as_dict())
            if confianza > mejor_confianza:
                mejor_confianza = confianza
                mejor_coincidencia = vocal

        if mejor_coincidencia and mejor_confianza > 0.7:
            HistorialReconocimientoVocal.objects.create(
                vocal_reconocida=mejor_coincidencia,
            ...
            return JsonResponse({
                ...
                'vocal_reconocida': {
                    ...
                    'vocal_vinculada': mejor_coincidencia.vocal_vinculada,
            ...
            return JsonResponse({
                'success': False,
                'error': 'No se pudo reconocer la vocal con suficiente confianza',
...
def vocales_capturadas(request):
    """API para obtener todas las vocales capturadas"""
    vocales = VocalCapturada.objects.filter(activo=True).order_by('vocal_vinculada')
    vocales_data = []
    for vocal in vocales:
        vocales_data.append({
            'id': vocal.id,
            'vocal_vinculada': vocal.vocal_vinculada,
            'tipo_mano': vocal.tipo_mano,
            'nombre_display': vocal.nombre_display,
            'landmarks': json.loads(vocal.landmarks_data) if vocal.landmarks_data else [],
            'landmarks_izquierda': json.loads(vocal.landmarks_mano_izquierda) if vocal.landmarks_mano_izquierda else None,
            'landmarks_derecha': json.loads(vocal.landmarks_mano_derecha) if vocal.landmarks_mano_derecha else None,
            'numero_muestras': vocal.numero_muestras,
            'fecha_creacion': vocal.fecha_creacion.isoformat(),
            'precision': vocal.precision_entrenamiento
        })
    return JsonResponse({'success': True,'vocales': vocales_data,'total': len(vocales_data)})

@csrf_exempt
@require_http_methods(["POST"])
def eliminar_gesto(request, vocal_id):
    """API para eliminar una vocal entrenada"""
    vocal = get_object_or_404(VocalCapturada, id=vocal_id)
    nombre_vocal = vocal.nombre_display
    vocal.delete()
    return JsonResponse({'success': True,'message': f'Vocal "{nombre_vocal}" eliminada exitosamente'})

@require_http_methods(["GET"])
def estadisticas_practica(request):
    """API para obtener estadísticas de práctica de vocales"""
    practicas = PracticaVocal.objects.all()
    ...
    estadisticas_por_vocal = {}
    for vocal in TipoVocal.choices:
        vocal_code = vocal[0]
        vocal_practicas = practicas.filter(vocal_objetivo=vocal_code)
        vocal_correctas = vocal_practicas.filter(fue_correcta=True).count()
        vocal_total = vocal_practicas.count()
        
        if vocal_total > 0:
            vocal_precision = (vocal_correctas / vocal_total) * 100
        else:
            vocal_precision = 0
            
        estadisticas_por_vocal[vocal_code] = {
            'total': vocal_total,
            'correctas': vocal_correctas,
            'precision': vocal_precision
        }
    ...
    return JsonResponse({
        ...
        'estadisticas': {
            ...
            'por_vocal': estadisticas_por_vocal
        }
    })
...
def reconocer_dos_manos(request):
    """
    API para reconocer simultáneamente ambas manos para vocales
    """
    try:
        if not request.body:
            return JsonResponse({'success': False, 'error': 'No se enviaron datos en la petición'}, status=400)
        ...
        # Consultar vocales entrenadas por mano
        vocales_izq = VocalCapturada.objects.filter(activo=True, tipo_mano='left')
        vocales_der = VocalCapturada.objects.filter(activo=True, tipo_mano='right')

        if not vocales_izq.exists() and not vocales_der.exists():
            return JsonResponse({'success': False, 'error': 'No hay vocales entrenadas para manos izquierda/derecha'}, status=400)

        # Buscar mejor coincidencia por mano
        umbral = 0.7
        mejor_izq, conf_izq = None, 0.0
        if left_points and vocales_izq.exists():
            for vocal in vocales_izq:
                c = calcular_similitud_landmarks(left_points, vocal.get_landmarks_as_dict())
                if c > conf_izq:
                    conf_izq, mejor_izq = c, vocal

        mejor_der, conf_der = None, 0.0
        if right_points and vocales_der.exists():
            for vocal in vocales_der:
                c = calcular_similitud_landmarks(right_points, vocal.get_landmarks_as_dict())
                if c > conf_der:
                    conf_der, mejor_der = c, vocal

        # Construir respuesta de vocales detectadas
        vocal_izq = mejor_izq.vocal_vinculada if mejor_izq and conf_izq > umbral else None
        vocal_der = mejor_der.vocal_vinculada if mejor_der and conf_der > umbral else None

        if vocal_izq is None and vocal_der is None:
            return JsonResponse({
                'success': False, 
                'error': 'No se detectaron vocales con suficiente confianza', 
                'confianzas': {'izquierda': conf_izq, 'derecha': conf_der}
            }, status=200)

        # Registrar en historial por mano si corresponde
        if mejor_izq and vocal_izq is not None:
            HistorialReconocimientoVocal.objects.create(
                vocal_reconocida=mejor_izq,
                confianza=conf_izq,
                landmarks_reconocidos=json.dumps(left_points)
            )
        if mejor_der and vocal_der is not None:
            HistorialReconocimientoVocal.objects.create(
                vocal_reconocida=mejor_der,
                confianza=conf_der,
        ...
        respuesta = {
            ...
            'manos_detectadas': {
                'izquierda': {
                    'vocal': vocal_izq,
                    'confianza': conf_izq,
                    'vocal_id': mejor_izq.id if mejor_izq else None,
                    'nombre': mejor_izq.nombre_display if mejor_izq else None,
                },
                'derecha': {
                    'vocal': vocal_der,
                    'confianza': conf_der,
                    'vocal_id': mejor_der.id if mejor_der else None,
...
def gestos_entrenados(request):
    """API para obtener todos los gestos entrenados de vocales"""
    gestos = VocalCapturada.objects.filter(activo=True).order_by('vocal_vinculada')
    gestos_data = []
    for gesto in gestos:
        gestos_data.append({
            'id': gesto.id,
            'vocal_vinculada': gesto.vocal_vinculada,
